...
//				memberTabbedPane.setSelectedIndex(1);
		...
		*/
		...
		memberJoinBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					int selectedAgeIndex = joinAgeComboBox.getSelectedIndex();
					if (selectedAgeIndex == 0) {
						ageMsgLB.setText("나이를 선택하세요");
						joinAgeComboBox.requestFocus();
						return;
					}
					String ageStr = (String) joinAgeComboBox.getSelectedItem();
					int age = Integer.parseInt(ageStr);
					
					boolean isMarried = joinMarriedCheckBox.isSelected();
					String married = "";
					if (isMarried) {
						married = "T";
					} else {
					...
					boolean isAdd = memberService.addMember(new Member(id, password, name, address, age, married, null));
					if (isAdd) {
						//가입성공 --> 로그인 화면으로 전환
						memberTabbedPane.setSelectedIndex(1);
					} else {
						//가입실패 --> 아이디 중복
						JOptionPane.showMessageDialog(null, id + "는 중복된 아이디입니다.");
						joinIdTF.requestFocus();
		...
		updateBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					String password = infoPassTF.getText();
					String name = infoNameTF.getText();
					String address = infoNameTF.getText();
					int age = Integer.parseInt((String) infoAgeComboBox.getSelectedItem());
					String isMarried = infoMarriedCheckBox.isSelected() ? "T" : "F";
					Member updateMember = new Member(id, password, name, address, age, isMarried, null);
		...
		logoutProcess(); // 처음 실행할 땐 로그아웃 상태
